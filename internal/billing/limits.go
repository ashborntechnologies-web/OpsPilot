...
// CheckProjectLimit returns ErrLimitReached when the org is at its plan's project cap.
// Limits are enforced per workspace (org), not per user (ADR-017).
func (s *Service) CheckProjectLimit(ctx context.Context, orgID uuid.UUID) error {
	var plan string
	var count int
	err := s.db.Pool.QueryRow(ctx, `
		SELECT o.plan, (SELECT COUNT(*) FROM projects WHERE org_id = o.id)
	...
	if count >= limits.MaxProjects {
		return &ErrLimitReached{Message: fmt.Sprintf(
			"This workspace's %s plan allows %d project(s) — upgrade to create more.", plan, limits.MaxProjects)}
	}
	return nil
}

// IncrementAIAction atomically meters one AI action against the org's monthly allowance,
// resetting the counter when the 30-day window has elapsed. Returns ErrLimitReached when
// the allowance is exhausted. Metered per workspace (org), not per user (ADR-017).
func (s *Service) IncrementAIAction(ctx context.Context, orgID uuid.UUID) error {
	// Reset stale windows first (no-op when within the window).
	_, err := s.db.Pool.Exec(ctx, `
		UPDATE organizations SET ai_actions_this_month = 0, ai_actions_reset_at = NOW()
		WHERE id = $1 AND ai_actions_reset_at < NOW() - INTERVAL '30 days'`, orgID)
	if err != nil {
		return err
	}
	...
	var used int
	err = s.db.Pool.QueryRow(ctx, `
		UPDATE organizations SET ai_actions_this_month = ai_actions_this_month + 1
		WHERE id = $1
		RETURNING plan, ai_actions_this_month`, orgID,
	).Scan(&plan, &used)
	if err != nil {
		return err
	}
	...
	if used > limits.AIActionsPerMonth {
		...
		s.db.Pool.Exec(ctx, `UPDATE organizations SET ai_actions_this_month = ai_actions_this_month - 1 WHERE id = $1`, orgID)
		return &ErrLimitReached{Message: fmt.Sprintf(
			"This workspace has used all %d AI actions on the %s plan this month — upgrade for unlimited AI.",
			limits.AIActionsPerMonth, plan)}
	}
	return nil
}

// Usage is the current consumption snapshot for a workspace (org), served by /users/me
...
// GetUsage returns the org's plan and consumption.
func (s *Service) GetUsage(ctx context.Context, orgID uuid.UUID) (*Usage, error) {
	var u Usage
	err := s.db.Pool.QueryRow(ctx, `
		SELECT o.plan, o.ai_actions_this_month, o.ai_actions_reset_at,
